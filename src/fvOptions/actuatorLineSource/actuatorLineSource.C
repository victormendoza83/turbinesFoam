...
bool Foam::fv::actuatorLineSource::read(const dictionary& dict)
{
    if (cellSetOption::read(dict))
    {
        ...
        TIPSR_ = pitchDict.lookupOrDefault("TIPSR", 0.0);
...
//	scalar deltaPitch = pitchAmplitude_/180.0*pi*(sin(omega*t) - sin(omega*(t - dt))); 	//harmonic

	scalar deltaPitch =  atan(cos(omega*t-0.5*Foam::constant::mathematical::pi)/(TIPSR_+sin(omega*t-0.5*Foam::constant::mathematical::pi)))-atan(cos(omega*(t - dt)-0.5*Foam::constant::mathematical::pi)/(TIPSR_+sin(omega*(t-dt)-0.5*Foam::constant::mathematical::pi))) ;  //pitchAmplitude_/180.0*pi*(sin(omega*t) - sin(omega*(t - dt))); 	//not - harmonic
